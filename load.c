...
@进入模式：
  edl（9008）
    ...
    NOHLOS\BOOT.BF.3.3\boot_images\core\wiredconnectivity\qusb\src\dci\qusb_dci_8953.c qusb_forced_download_feature_supported = TRUE; TRUE修改为FALSE
...
3其他应用修改了sdcard目录的system用户组权限，导致APK访问异常（uid）
